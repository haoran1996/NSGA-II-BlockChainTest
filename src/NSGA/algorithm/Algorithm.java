...
import NSGA.api.GraphPlot;
import NSGA.api.Reporter;
import NSGA.api.Service;
import NSGA.api.Synthesis;
...
import static blockchain.CMDRedirect.Main.getSql_statement;
...
public class Algorithm {
    ...
    public static void main(String[] args) {
        Main.ProjectPath = "F:\\blockchain\\Test20181124\\Etherchicks";
        Main.TableName = "Etherchicks";
        Main.DirPath = "F:\\blockchain\\Test20181124\\Etherchicks\\test";
        Main.sql_statement = getSql_statement(Main.TableName);
...
//        Population parent = Service.nonDominatedPopulationSort(Synthesis.syntesizePopulation());
        ...
        Population parent = Service.nonDominatedPopulationSort(new Population(populace));
        ...
        for(int i = 2; i <= Configuration.getGENERATIONS(); i++) {
            ...
            for(int j = 1; j <= rankedFronts.size(); j++) {
                ...
                if(singularFront != null && !singularFront.isEmpty() && usableSpace > 0) {
                    ...
                    else {//us < singularFront.size.....4<5
...
//            multiPlotGraph.prepareMultipleDataset(child, i, "generation " + i);
        ...
        HashMap<Integer, List<Chromosome>> finalrankedFronts = Service.fastNonDominatedSort(Service.createCombinedPopulation(parent, child));
        System.out.format(" Gas    uncov   time %n");
        for (Chromosome c : finalrankedFronts.get(1)) {
            double uncov = Configuration.getObjectives().get(1).objectiveFunction(c);
            System.out.println(
                    Configuration.getObjectives().get(0).objectiveFunction(c) + "   " +
                            uncov  + "   " + Configuration.getObjectives().get(2).objectiveFunction(c));
        }
//        /**
